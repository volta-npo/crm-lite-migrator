...
### A migration checklist for moving customer lists from notebooks, spreadsheets, or inboxes into a lightweight CRM.
...
**AI & Automation** · **Repo 33/50** · **No backend. No login. Client data stays local.**

[Live app](https://volta-npo.github.io/33-crm-lite-migrator/) · [Report an issue](https://github.com/volta-npo/33-crm-lite-migrator/issues) · [Volta](https://voltanpo.org)
...
**CRM Lite Migrator** is a polished, local-first open-source tool from Volta's 50-project OSS division. It helps Tech/ops pods cleaning client data turn real community work into structured evidence, client-safe handoffs, and mentor-reviewable release packets.
...
Small businesses often do not need a complex CRM, but they do need clean contacts and a repeatable import process.

### The niche

First CRM setup for very small teams.

### North-star metric

`clean contacts migrated with consent status`
...
flowchart TD
    ...
    Evidence --> Workbench["CRM Lite Migrator"]
    Workbench --> Score["Automation safety"]
    Score --> Cert["v3 production certification"]
    Cert --> A1["Clean CRM CSV"]
    Cert --> A2["Rejected rows report"]
    Cert --> A3["Mapping doc"]
...
flowchart LR
    subgraph Modules["First-class modules"]
        M1["Source inventory"]
        M2["Field mapping sheet"]
        M3["Duplicate review process"]
        M4["Owner handoff tutorial"]
    end
    M1 --> Workbench
    M2 --> Workbench
    M3 --> Workbench
...
sequenceDiagram
    ...
    participant T as CRM Lite Migrator
...
|---|---|
| **Domain workbench** | A purpose-built crm cleaner interface for a migration checklist for moving customer lists from notebooks, spreadsheets, or inboxes into a lightweight crm. |
...
|---:|---|---|
| 1 | **Source inventory** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 2 | **Field mapping sheet** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 3 | **Duplicate review process** | Converts field work into repeatable, reviewable Volta delivery evidence. |
| 4 | **Owner handoff tutorial** | Converts field work into repeatable, reviewable Volta delivery evidence. |
...
```bash
git clone https://github.com/volta-npo/33-crm-lite-migrator.git
cd 33-crm-lite-migrator
npm test
npm start
```
